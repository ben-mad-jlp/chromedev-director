...
export function analyzeVariableDependencies(test: SavedTest): DependencyAnalysis {
  ...
  // Helper to get a human-readable label for a step
  const getStepLabel = (step: StepDef): string => {
  ...
  // Helper to extract all $vars references from a step
  const extractVarReferences = (step: StepDef): string[] => {
  ...
  const processSection = (steps: StepDef[], sectionName: 'before' | 'steps' | 'after') => {
    for (let i = 0; i < steps.length; i++) {
      ...
      // Handle nested structures (loop, run_test)
      if ('loop' in step && step.loop.steps) {
        // Recursively process loop body
        // Note: This doesn't track section correctly for nested steps,
        // but gives us basic dependency info
        for (const nestedStep of step.loop.steps) {
          ...
          for (const varName of nestedRefs) {
            ...
            variables[varName].used_by.push({
              ...location,
              step_label: `${location.step_label} (loop body)`
            });
          }
...
export function checkRemoveStepSafety(
  ...
  if ('as' in step && step.as) {
    ...
    const isUsed = allSteps.some((s, idx) => {
